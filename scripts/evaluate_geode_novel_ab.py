"""Official GEODE scoring for Transient quota experiments."""
import argparse
import csv
import json
import statistics
...
p.add_argument('--tunnel',type=int,choices=range(1,6),default=2)
n=p.parse_args().tunnel
seq=f'geode_tunnel{n}'
base=e.BASE/'validation'/seq/'transient_novel_ab'
gt=e.ROOT/'bag/GEODE/Tunneling_tunnel_gamma'
out=e.BASE/'summary'/f'tunnel{n}_transient_novel_ab'
...
for group in ['control','control_off']:
    for i in range(1,4):
        ...
        if not ev.exists():
            e.evaluate(seq,'geode',gt/f'groudtruth/Tunneling_tunnel{n}.txt',gt/'gamma2GT_leica.py',gt/'rmse.py',
